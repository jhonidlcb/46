...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      yPos += 10;
      doc.fontSize(16).fillColor('#2563eb').font('Helvetica-Bold').text('¡Gracias por confiar en SoftwarePar!', leftMargin, yPos, { align: 'center', width: contentWidth });

      // Régimen Tributario - Corregido a lo que se ve en la imagen
      doc.fontSize(8).fillColor('#4b5563').font('Helvetica-Oblique');
      doc.text('Régimen Tributario: Servicios Digitales (Ley 125/91) - IVA Exento', leftMargin, yPos + 60, { align: 'center', width: contentWidth });
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const logoPath = path.resolve(process.cwd(), "attached_assets/image_1767293138533.png");
      try {
        if (fs.existsSync(logoPath)) {
          // Logo posicionado a la izquierda con espacio del borde
          doc.image(logoPath, leftMargin + 15, yPos + 15, { width: 140 });
        } else {
      ...
      doc.fontSize(14).fillColor('#1e3a8a').font('Helvetica-Bold').text('BOLETA ELECTRÓNICA', rightHeaderX, yPos + 15);
      doc.fontSize(8).fillColor('#475569').font('Helvetica').text('IVA General 10% / IRE SIMPLE', rightHeaderX, yPos + 35);
      ...
      // Subtotales e IVA
      yPos = 380;
      const subtotalX = leftMargin + contentWidth * 0.55; // Mover un poco más a la izquierda para dar espacio
      const valueColumnX = subtotalX + 110;
      ...
      doc.text(`PYG ${amountPYG.toLocaleString('es-PY')}`, valueColumnX, yPos + 70, { width: valueWidth - 10, align: 'right' });
